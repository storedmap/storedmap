...
public interface Driver {
     ...
     * 
     * <p>The driver should first check the existing lock in the underlying database, and if found, return the
     * time remaining for this lock to be automatically released.</p>
     * 
     * <p>If the remaining time is less or equals to zero, the lock should be considered expired and replaced with the
     * new one for the specified amount of time. The method should still return zero or negative value to indicate that 
     * the key was locked.</p>
     * 
     * <p>The lock can be removed before the specified amount of time passes using the method {@link unlock(String, String, Object)}</p>
     * 
     * 
     ...
     * 
     * <p>Commands the driver to remove the lock regardless of the maximum active time set in 
